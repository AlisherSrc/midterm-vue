<template>
   <div class="page-container">
      ...
      <header class="header">
         ...
         <nav class="nav-links">
            <NuxtLink to="/products" class="nav-link">ALL PRODUCTS</NuxtLink>
         ...
         <div class="header-actions">
            ...
            <NuxtLink to="/cart" class="cart-icon">
               <!-- Cart Icon -->
               <img src="@/assets/images/app-header/header-cart.svg" alt="cart">
            </NuxtLink>
         </div>
      </header>
...
// Helper function to determine if a product's rating falls within the selected range
function matchesRating(productRating, selectedRating) {
  if (!selectedRating) return true; // If no rating is selected, return true for all products
  if (selectedRating === 5) {
    return productRating === 5; // Fixed condition for 5/5, strictly equal to 5
  } else if (selectedRating === 4) {
    return productRating >= 4 && productRating < 5;
  } else if (selectedRating === 3) {
    return productRating >= 3 && productRating < 4;
  } else if (selectedRating === 2) {
    return productRating >= 2 && productRating < 3;
  } else if (selectedRating === 1) {
    return productRating >= 1 && productRating < 2;
  }
  return false;
}
...
const filteredProducts = computed(() =>
   products.value.filter((product) => {
      ...
      // Check if product matches the selected rating (using range logic)
      const matchesRatingRange = matchesRating(product.rating, selectedRating.value);

      // Return product if all conditions are met
      return matchesSearch && matchesCost && matchesCompany && matchesCategory && matchesRatingRange;
   })
);
